...
def process_pdfs(input_dir: str, output_dir: str) -> None:
    ...
    if not pdfs:
        logger.warning(f"⚠️ No PDFs found in input directory: {input_dir}")
        logger.info(f"📁 Directory contents: {os.listdir(input_dir)}")
        return

    logger.info(f"📚 Found {len(pdfs)} PDF(s): {pdfs}")
...
# ----------------------------
# Entrypoint - Fixed for local execution
# ----------------------------
def main():
    # Get the directory where the script is located
    script_dir = os.path.dirname(os.path.abspath(__file__))
    
    # Look for input and output directories relative to script location
    input_dir = os.path.join(script_dir, "input")
    output_dir = os.path.join(script_dir, "output")
    
    # If input directory doesn't exist, check if PDFs are in the same directory as script
    if not os.path.exists(input_dir):
        logger.info(f"📁 Input directory not found at {input_dir}")
        logger.info(f"🔍 Looking for PDFs in script directory: {script_dir}")
        
        # Check if there are PDFs in the script directory
        script_pdfs = [f for f in os.listdir(script_dir) if f.lower().endswith(".pdf")]
        if script_pdfs:
            logger.info(f"📚 Found PDFs in script directory: {script_pdfs}")
            input_dir = script_dir
        else:
            logger.warning("❌ No PDFs found in script directory either")
            return
